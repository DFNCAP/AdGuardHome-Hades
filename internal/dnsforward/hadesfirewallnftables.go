...
type firewallContext struct {
	...
	genericAllowedIPs map[string]bool            // ipDest -> true (for generic allowlist rules)
}
...
func initialiseFirewall(homeAddress, interfaceName string) *firewallContext {
	...
	f := &firewallContext{
		...
		genericAllowedIPs: make(map[string]bool),
	}
...
func (fctx *firewallContext) setFILTERTable(add bool) error {
	if add {
		...
		// First, add conntrack rule to allow return traffic for established connections
		// This is critical for generic allowlist rules to work without a client chain
		fctx.conn.AddRule(&nftables.Rule{
			Table: fctx.filterTable,
			Chain: fctx.forwardChain,
			Exprs: append(createCTStateRule("ESTABLISHED,RELATED"), &expr.Verdict{
				Kind: expr.VerdictAccept,
			}),
		})
...
func (fctx *firewallContext) openGenericRoute(ipDestinations []string) error {
	// Add generic allowlist rules directly to AdGuardFirewall_forward chain
	// These rules allow traffic to specific destinations from any client on wlan1

	for _, ipDest := range ipDestinations {
		// Check if this destination IP is already allowed generically
		if fctx.genericAllowedIPs[ipDest] {
			// Already allowed, skip adding duplicate rule
			log.Debug("Generic route already exists for %s", ipDest)
			continue
		}

		dstIP := net.ParseIP(ipDest)
		if dstIP == nil {
			log.Error("Invalid destination IP address: %s", ipDest)
			continue
		}

		var exprs []expr.Any

		// Match input interface (wlan1)
		exprs = append(exprs, &expr.Meta{Key: expr.MetaKeyIIFNAME, Register: 1})
		exprs = append(exprs, &expr.Cmp{
			Op:       expr.CmpOpEq,
			Register: 1,
			Data:     []byte(fctx.interfaceName + "\x00"),
		})
		...
		// Get current rules in forward chain to find insertion point
		// We want to insert after DNS rules but before client chain jumps
		rules, err := fctx.conn.GetRules(fctx.filterTable, fctx.forwardChain)
		if err != nil {
			log.Error("Failed to get forward chain rules: %v", err)
			return err
		}

		// Find the position after DNS rules (typically first 2 rules)
		// DNS rules are for port 53, so we insert after those
		insertPos := uint64(0)
		for i, rule := range rules {
			// Check if this rule is a jump to a client chain
			isJump := false
			for _, e := range rule.Exprs {
				if verdict, ok := e.(*expr.Verdict); ok {
					if verdict.Kind == expr.VerdictJump {
						isJump = true
						break
					}
				}
			...
			if isJump {
				// Insert before the first jump rule
				insertPos = uint64(i)
				break
			}
		}
		...
		if insertPos > 0 {
			// Insert at specific position (after DNS rules, before jumps)
			fctx.conn.InsertRule(&nftables.Rule{
				Table:    fctx.filterTable,
				Chain:    fctx.forwardChain,
				Exprs:    exprs,
			...
			// Append to end if no jump rules found
			fctx.conn.AddRule(&nftables.Rule{
				Table: fctx.filterTable,
				Chain: fctx.forwardChain,
				Exprs: exprs,
		...
		if err := fctx.conn.Flush(); err != nil {
			log.Error("Failed to add generic route rule for %s: %v", ipDest, err)
			return err
		}

		// Mark this IP as allowed generically
		fctx.genericAllowedIPs[ipDest] = true
		log.Info("Added generic route for destination: %s", ipDest)
	}
...
func (fctx *firewallContext) removeClientFunc(chainName string) error {
	...
	clientIP := parts[len(parts)-1]

	cmd := exec.Command("conntrack", "-D", "-s", clientIP)
	cmd.Run()
...
func processIPTables(ctx context.Context, dctx *dnsContext) (rc resultCode) {
	...
	isGenericAllowList := false
	if dctx.result != nil && dctx.result.Reason == filtering.NotFilteredAllowList {
		isGenericAllowList = true
		for _, rule := range dctx.result.Rules {
			dctx.srv.logger.InfoContext(ctx, rule.Text)
			if strings.Contains(rule.Text, "$client=") {
				isGenericAllowList = false
				break
			}
		}
	...
	// Handle generic allowlist rules (no client restriction)
	if isGenericAllowList {
		dctx.srv.logger.InfoContext(ctx, "NFTABLES - Generic allowlist detected")

		if len(ipDestinations) == 0 {
			dctx.srv.logger.InfoContext(ctx, "NFTABLES - NO: No destination IPs to add for generic rule")
			return resultCodeSuccess
		}

		dctx.srv.logger.InfoContext(ctx, "NFTABLES - Adding generic rules to AdGuardFirewall_forward chain")
		for _, destIP := range ipDestinations {
			dctx.srv.logger.InfoContext(ctx, fmt.Sprintf("NFTABLES - Generic rule destination: %s", destIP))
		}

		// Add rules directly to AdGuardFirewall_forward without client chain
		if dctx.srv.firewall != nil {
			if err := dctx.srv.firewall.openGenericRoute(ipDestinations); err != nil {
				log.Error("Failed to add generic routes: %v", err)
	...
	// Client-specific rules require all information
	...
	// Now we open the firewall for each (client-specific)
	dctx.srv.logger.InfoContext(ctx, "NFTABLES - Client-specific rule for %s (%s)", clientName, ipSource)
	if dctx.srv.firewall != nil {
		for _, d := range ipDestinations {
			dctx.srv.logger.InfoContext(ctx, "NFTABLES - Adding client rule: %s -> %s", ipSource, d)
